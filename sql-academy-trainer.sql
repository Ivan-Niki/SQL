...
функция вернёт значение второго аргумента значение_1, иначе возвращается значение третьего аргумента значение_2.
Пример использования с реальной БД. 
Необходимо на основании цены определить принадлежность жилья к одному из двух классов: "Комфорт-класс" и "Эконом-класс". Если цена больше или равна 150, то это жильё относится к "Комфорт-класс". */

SELECT id, price,
    IF(price >= 150, "Комфорт-класс", "Эконом-класс") AS category
FROM Rooms;
/* В выводе будет три столбца: id, price и category. В столбце category в зависимости от цены будут значения "Комфорт-класс" или "Эконом-класс". */
