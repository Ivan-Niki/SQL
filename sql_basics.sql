...
/* С помощью оператора WHERE мы можем указать, какие строки следует удалить. Однако если нужно удалить сразу все строки, то достаточно просто не указывать оператор WHERE.
...
Эти команды управляют обработкой транзакций в базе данных. Например, откат, сохранение, фиксация. 
Эти команды оказывают постоянное влияние на базу данных. Например, откат всей транзакции к предыдущему состоянию, даже если один процесс завершается с ошибкой.
...
/* 1. COMMIT: команда COMMIT используется для сохранения всех изменений, сделанных в БД в текущей транзакции.
Синтаксис: COMMIT;
Пример: */
DELETE FROM CUSTOMERS
WHERE AGE = 25;
...
/* 2. ROLLBACK: Команда отката используется для отмены изменений, сделанных в БД в текущей транзакции.
Синтаксис: ROLLBACK;
Пример: */
DELETE FROM CUSTOMERS
WHERE AGE = 25;
...
/* 3. SAVEPOINT: используется для отката транзакции до определенной точки без отката всей транзакции, т.е.
для определения идентификатора, указывающего на именованную точку сохранения транзакции.
Синтаксис: */
...
--------- DCL – Data Control Language (Язык управления транзакциями) ---------

/*Data Control Language (DCL) – группа операторов определения доступа к данным. Иными словами, это операторы для управления разрешениями, с помощью них мы можем разрешать или запрещать выполнение определенных операций над объектами базы данных. 

Сюда входят: 

GRANT – используется для предоставления пользователю или группе прав доступа к базе данных (разрешения на определённые операции с объектом);
