...
Сюда входят три типа БД, которые всё ещё могут встречаться в специализированных средах, но в основном заменены 
надежными и производительными альтернативами. К простейшим БД относятся три вида баз данных:

--> 1)Текстовые файлы
Первый и простейший способ хранения данных – текстовые файлы. Метод применяется и сегодня для работы с 
небольшими объёмами информации. Для разделения полей используется специальный символ: запятая или точка с 
запятой в csv-файлах датасетов, двоеточие или пробел в *nix-подобных системах.

--> 2) Иерархические базы данных
Простейшая структура, где записи, как ветви, отходят от «родителя». Информация связана по аналогии с папками 
на рабочем столе. У каждой записи есть физическая связь только с одной предыдущей, а отношение многих ко 
многим невозможно.
В отличие от текстовых таблиц, в иерархических БД появляются связи между объектами. В иерархических базах 
данных каждая запись имеет одного «родителя». Это создаёт древовидную структуру, в которой записи 
классифицируются по их отношениям с цепочкой родительских записей.
...
========== Есть два наиболее распространенных типа баз данных: ==========
II. --> Реляционные базы данных (базы данных SQL).
III.  --> Нереляционные (базы данных NoSQL).
...
Популярные реляционные СУБД: MySQL, PostgreSQL, MSSQL, Oracle, SQLite, MariaDB и др.
...
В статьях и книгах в качестве типового примера работы таких БД часто приводят сценарий загрузки страницы 
пользователя в социальной сети. В этом случае у человека есть основные данные: ФИО, дата рождения, пол — и 
дополнительная информация, которая может включать в себя несколько элементов и требовать разной структуры. 
Например, места учёбы, проживания, работы.


--> 2) Key-value-хранилища (хранение в виде «ключи-значения»)
Key-value-хранилища (Redis, Aerospike, DynamoDB; различные хранилища кэша – например, Memcached и другие) хранят данные в виде хеш-таблицы. 
...
Примеры графовых баз данных: Neo4J, JanusGraph, Dgraph, OrientDB.


--> 4) Колоночные базы данных
Подобно реляционным, в этих базах данные хранятся в виде таблиц. Но структура колонок строго не 
регламентирована — они могут объединяться в колоночные семейства с определённым форматом. Строки колоночного 
семейства имеют уникальные идентификаторы.
...
Колоночные БД удобны, когда нас интересует информация не во всех столбцах, а в каких-то конкретных. Искать по 
конкретным столбцам быстрее, чем по строкам.

Примеры колоночных баз данных: Cassandra, HBase, ClickHouse.
...
/* Вывести все данные из таблицы superheroes и отсортировать их по столбцу appearances в порядке убывания, 
т.е. сначала будут идти супергерои, у которых значение в столбце appearances максимальное */
SELECT * 
FROM superheroes
...
LEFT JOIN Orders
ON Customers.customer_id = Orders.customer;
/* Здесь мы выбираем столбцы customer_id и first_name (из таблицы Customers) и столбец amount (из таблицы 
Orders). В результате получаем те строки, в которых есть совпадение между customer_id (таблицы Customers) и 
customer (таблицы Orders) вместе со всеми остальными строками из (левой) таблицы Customers. */
...
/* В SQL мы можем поместить один запрос внутри другого запроса (сделав подзапрос). 
В подзапросе результат внешнего запроса зависит от результатов внутреннего запроса. 
Вот почему подзапросы также называют вложенными запросами. Подзапросы оформляются в круглых скобках.
Например: */
SELECT *
FROM Customers
WHERE age = (
...
/* Здесь сначала выполняется подзапрос (внутренний запрос) — мы выбираем наименьшее значение age из таблицы Customers. 
Затем выполняется внешний запрос — мы выбираем строки, в которых age равен результату подзапроса (наименьшему значению age). */

/*Рассмотрим другой пример. Предположим, нам нужны сведения о клиентах, разместивших заказ. 
Вот как мы можем это сделать с помощью подзапроса: */
...
В его определении вы можете указать, что значение данного столбца должно удовлетворять логическому выражению (проверке истинности). 
Ограничение-проверка состоит из ключевого слова CHECK, за которым идёт выражение в скобках. 
Это выражение должно включать столбец, для которого задаётся ограничение, иначе оно не имеет большого смысла. Например, цену товара можно ограничить положительными значениями так: */
...
Чтобы создать именованное ограничение, напишите ключевое слово CONSTRAINT, а за ним идентификатор и собственно определение ограничения. 
...
идентификатором строк в таблице. Для этого требуется, чтобы значения были одновременно уникальными и отличными 
...
/* Давайте предположим, что у вас есть таблица с заказами этих продуктов. Мы хотим, чтобы в таблице заказов 
содержались только заказы действительно существующих продуктов. Поэтому мы определим в ней ограничение 
внешнего ключа, ссылающееся на таблицу продуктов: */
CREATE TABLE orders (
...
Транзакция – это команда или блок команд (инструкций), которые успешно завершаются как единое целое, при этом 
в базе данных все внесенные изменения фиксируются на постоянной основе или отменяются, т.е. все изменения, 
внесенные любой командой, входящей в транзакцию, будут отменены.
Важным моментом транзакции является то, что она объединяет несколько шагов в единую операцию «все или ничего». 
Промежуточные состояния между шагами не видны другим параллельным транзакциям, и если происходит какой-то 
сбой, препятствующий завершению транзакции, то ни один из шагов вообще не влияет на базу данных.
...
/* 3. SAVEPOINT (дословно - "точка сохранения"). Используется для отката транзакции до определенной точки без 
отката всей транзакции, т.е. для определения идентификатора, указывающего на именованную точку сохранения транзакции.
Операторы в транзакции можно контролировать более детально, используя точки сохранения. Точки сохранения 
позволяют вам выборочно отбрасывать части транзакции, сохраняя при этом остальные. После определения точки 
сохранения с помощью SAVEPOINT, вы можете при необходимости вернуться к точке сохранения с помощью ROLLBACK TO. 
Все изменения базы данных транзакции между определением точки сохранения и откатом к ней отбрасываются, но изменения до точки сохранения сохраняются.
Синтаксис: */
...
/* Data Control Language (DCL) – группа операторов определения доступа к данным. Иными словами, это операторы 
для управления разрешениями, с помощью них мы можем разрешать или запрещать выполнение определенных операций 
...
/* 
------------ Конкатенация строк ------------
Часто в запросах приходится объединять несколько значений в одно текстовое значение (конкатенация). Например, 
может потребоваться получить строку следующего формата: 'текущее время: 2019-02-19 15:28:44.049702+07'. */
...
Денормализация обычно проводится путем добавления избыточных данных в таблицу, т.е. тех данных, 
которые по требованиям той или иной нормальной формы должны выноситься в отдельную таблицу.

Например, в процессе эксплуатации базы данных у Вас возникла необходимость добавить новую характеристику, 
но по требованиям определенной нормальной формы Вы должны под эту характеристику создать новую таблицу, 
однако требования предметной области и требования разрабатываемой системы позволяют этого не делать, 
при этом Вы четко понимаете, что невынесение этих данных в отдельную таблицу положительно скажется на эксплуатации базы данных. В этом случае Вы выполняете как раз денормализацию.
Также денормализация проводится и в процессе проектирования базы данных, например, когда по требованиям той или иной нормальной формы Вы должны выполнить декомпозицию таблицы, 
а Вы ее намеренно не выполняете, все по тем же причинам, т.е. это удовлетворяет Вашим требованиям.
